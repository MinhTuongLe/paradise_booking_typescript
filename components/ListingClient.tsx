...
import { ConfigType, BookingMode, Role, ReportTypes, Topic } from "@/enum";
              ...
              <div className="my-8 w-full">
                <>
                  ...
                  <div className="mt-4 gap-8 flex flex-nowrap overflow-x-scroll review-horizontal pb-1">
                    {place?.post_guide_relates &&
                    !isEmpty(place.post_guide_relates) ? (
                      place.post_guide_relates.map((post) => (
                        <div key={place.id} className="w-[15%] flex-shrink-0">
                          <PostGuiderCardVertical data={post} />
                        </div>
                      ))
                    ) : (
                      <div className="font-bold text-lg text-rose-500">
                        {t("place-feature.no-nearby-post-guider")}
                      </div>
                    )}
                  </div>
                </>
