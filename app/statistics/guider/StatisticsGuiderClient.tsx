...
import PopupPostGuideTable from "@/components/statistics/PopupPostGuideTable";
...
interface StatisticsGuiderClientProps {
  posts: PostGuider[];
  paging: Pagination;
...
function StatisticsGuiderClient({
  ...
  const [selectedRoom, setSelectedRoom] = useState<string>(
    params?.get("post_guide_id") || ""
  );
  const [keyword, setKeyword] = useState(params?.get("post_guide_id") || "");
  ...
  // update url khi nhập input post guide id
  const updateURLInSearch = (keyword: string) => {
    ...
    updatedQuery = {
      ...currentQuery,
      post_guide_id: keyword ? keyword : null,
    };

    const url = qs.stringifyUrl(
      {
        url: pathName || "/statistics/guider",
        query: updatedQuery,
      },
      { skipNull: true }
    );
            ...
            <div>
              ...
              <div ref={popupSearchRef} className="relative">
                <input
                  ...
                  placeholder={t("statistic-feature.search-post-guide-id")}
                />
                <button
                ...
                {isShowPopup && (
                  ...
                  >
                    <PopupPostGuideTable
                      ...
                      ref={tablePopupDataRef}
                      handleSelectPostGuider={(postGuider: string) => {
                        setIsShowPopup(false);
                        updateURLInSearch(postGuider);
                        setKeyword(postGuider);
                        setSelectedRoom(postGuider);
                      }}
                    />
                  </div>
                )}
